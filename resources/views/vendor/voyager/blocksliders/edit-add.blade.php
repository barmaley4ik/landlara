...
@section('content')
    ...
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form" action="@if(isset($dataTypeContent->id)){{ route('voyager.blocksliders.update', $dataTypeContent->id) }}@else{{ route('voyager.blocksliders.store') }}@endif" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-8">
                    <div class="panel panel panel-bordered">
                        <div class="panel-body">
                            ...
                            <div class="css_custom">
                                @foreach($dataTypeRows as $row)
                                    @if($row->field == 'css_custom')
                                        @php
                            ...
                            <div class="text">
                                @foreach($dataTypeRows as $row)
                                    @if($row->field == 'blockslider_belongstomany_oneslider_relationship')
                                        @php
                                            $options = json_decode($row->details);
                                            $display_options = isset($options->display) ? $options->display : NULL;
                                        @endphp
